...
class DQN_On_Off(RLAlgorithmBase):
    name = "dqn-on-off"
    ...
    def critic_loss(
        ...
        next_states=None,  # used in markov_critic
    ):
        # Q^tar(h(t+1), pi(h(t+1))) + H[pi(h(t+1))]
        with torch.no_grad():
            if markov_critic:  # (B, A)
                next_v = critic(next_observs, next_states)
                next_target_v = critic_target(next_observs, next_states)
            else:
                next_v = critic(
