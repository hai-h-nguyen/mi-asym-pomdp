...
class DQN_BA(RLAlgorithmBase):
    """A version of biased asymmetric DQN that uses DDQN instead of pure DQN
    ...
    def critic_loss(
        ...
        with torch.no_grad():
            ...
            # q_target: (T, B, 1)
            qz_target = rewards + (1.0 - dones) * gamma * next_target_qhz  # next q
            qz_target = qz_target[1:]  # (T, B, 1)
            qh_target = next_target_vhz[1:]  # (T, B, 1)

        # Q(h(t), a(t)) (T, B, 1)
        vh_pred, vz_pred = critic(
        ...
        qz_pred = vz_pred.gather(
            dim=-1, index=stored_actions
        )  # (T, B, A) -> (T, B, 1)

        return (qh_pred, qh_target), (qz_pred, qz_target)
