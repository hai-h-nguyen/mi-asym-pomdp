...
from policies.models.ais import LatentModel
...
class Actor_RNN(nn.Module):
    def __init__(
        ...
        self.seq_model_target = deepcopy(self.seq_model)
        self.dyn_latent_model = LatentModel(self.seq_model.hidden_size,
                                            action_dim,
                                            128
                                            )
        self.zp_loss_w = config_actor.zp_loss_w
    ...
    def _get_shortcut_obs_embedding(self, observs):
        if self.image_encoder is None:  # vector obs
            out = self.current_observ_embedder(observs)
        else:  # pixel obs
            out = self.image_encoder(observs)

        return out

    @torch.no_grad()
    def get_target_hidden_states(
        ...
        assert initial_internal_state is None  # training
        initial_internal_state = self.seq_model_target.get_zero_internal_state(
            batch_size=inputs.shape[1]
        )  # initial_internal_state is zeros
        output, _ = self.seq_model_target(inputs, initial_internal_state)
        return output
    ...
    def forward(self, prev_actions, rewards, observs):
        ...
        aux_info = {}
        dyn_input = torch.cat((hidden_states[:-1], prev_actions[1:]), dim=-1)
        pred_next_latent = self.dyn_latent_model(dyn_input)
        aux_info["pred_next_latent"] = pred_next_latent
        target_next_latent = self.get_target_hidden_states(prev_actions,
                                                           rewards,
                                                           observs)
        target_next_latent = target_next_latent[1:]
        aux_info["target_next_latent"] = target_next_latent
        aux_info["zp_loss_w"] = self.zp_loss_w
