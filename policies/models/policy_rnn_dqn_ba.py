...
from policies.models.recurrent_critic_ba import Critic_RNN


class ModelFreeOffPolicy_DQN_RNN_BA(nn.Module):
    ARCH = "memory"
    Markov_Actor = False
    Markov_Critic = False
    ...
    def forward(self, actions, rewards, observs, states, dones, masks):
        ...
        ### 1. Critic loss
        (qh_pred, qh_target), (qz_pred, qz_target) = self.algo.critic_loss(
        ...
        qz_pred = qz_pred * masks
        qz_target = qz_target * masks
        qf_loss += 0.5 * ((qz_pred - qz_target) ** 2).sum() / num_valid  # TD error
