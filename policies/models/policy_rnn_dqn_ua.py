...
from policies.models.recurrent_critic_ua import Critic_RNN


class ModelFreeOffPolicy_DQN_RNN_UA(nn.Module):
    ARCH = "memory"
    Markov_Actor = False
    Markov_Critic = False
    ...
    def forward(self, actions, rewards, observs, states, dones, masks):
        ...
        ### 1. Critic loss
        (qh_pred, qh_target), (qhz_pred, qhz_target) = self.algo.critic_loss(
        ...
        qh_pred = qh_pred * masks
        qh_target = qh_target * masks
        qf_loss = 0.5 * ((qh_pred - qh_target) ** 2).sum() / num_valid  # TD error

        qhz_pred = qhz_pred * masks
        qhz_target = qhz_target * masks
        qf_loss += 0.5 * ((qhz_pred - qhz_target) ** 2).sum() / num_valid  # TD error
        ...
        outputs.update({
            ...
            "qh": (qh_pred.sum() / num_valid).item(),
            "qhz": (qhz_pred.sum() / num_valid).item(),
        })
