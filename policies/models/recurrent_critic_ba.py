...
class Critic_RNN(nn.Module):
    """Used for DQN_BA (discrete action) and BA-SAC (continuous action)
    ...
    def __init__(
        ...
        qzf = self.algo.build_critic(
            input_size=shortcut_embedding_size,
            hidden_sizes=config_critic.hidden_dims,
            action_dim=action_dim,
        )
    ...
    def forward(self, prev_actions, rewards, observs, states, current_actions):
        ...
        curr_state_embed = torch.cat((state_embeds, curr_embed),
                                     dim=-1)

        # q values
        qh = self.qhf(joint_embeds)
        qz = self.qzf(curr_state_embed)
