...
class ModelFreeOffPolicy_DQN_RNN_ZP(nn.Module):
    ARCH = "memory"
    Markov_Actor = False
    Markov_Critic = False
    ...
    def forward(self, actions, rewards, observs, dones, masks):
        ...
        pred = info["pred_next_latent"] * masks
        target = info["target_next_latent"] * masks
        zp_loss = ((pred - target)**2).sum() / num_valid
        zp_loss *= info["zp_loss_w"]
        qf_loss += zp_loss
        outputs.update({
