...
class SphinxEnv(gym.Env):
    def __init__(self, seed=0, rendering=False):
        ...
        # Create grid world
        self.grid_len = 6
        self.mid_len = self.grid_len // 2
        state_mat = np.ones((self.grid_len, self.grid_len))
        self.state_mat = state_mat
        self.goal_lists = [
            self.grid_len - 1,
            self.grid_len * self.grid_len - 1,
            self.grid_len * self.grid_len - self.grid_len,
        ]
        ...
        self.low_state = np.array([0.0, 0.0, -1.0, -1.0], dtype=np.float32)
        self.high_state = np.array([1.0, 1.0, 1.0, 1.0], dtype=np.float32)
        ...
        if self.obs_type == IMAGE_NORMED_FLATTEN:
            self.observation_space = gym.spaces.Box(
                shape=(np.array(self.img_size).prod(),),
                low=0,
                high=1.0,
                dtype=np.float32,
            )
            ...
            self.observation_space = gym.spaces.Box(
                shape=(np.prod(self.img_size),), low=0, high=1.0, dtype="float32"
            )
            self.state_space = gym.spaces.Box(
                shape=(np.prod(self.img_size),), low=0, high=1.0, dtype="float32"
            )
            self.image_space = gym.spaces.Box(
                shape=self.img_size, low=0, high=1.0, dtype="float32"
            )
        else:
            self.observation_space = gym.spaces.Box(
    ...
    def step(self, action):
        ...
        if self._state in self.goal_lists:
            done = True
        ...
        self.state = self._prepare_obs(self._state, self.goal_loc)

        if self.visualize:
            time.sleep(0.1)

        info = {}
        info["success"] = reward != self.step_reward and reward != self.ask_reward

        ask_reward = self.ask_reward if enter_info_area else 0.0

        return obs, reward + ask_reward, done, info

    def render(self, mode="human"):
        pass

    def _check_far_enough(self, state, goals):
    ...
    def reset(self):
        ...
        self.goal_loc = random.choice(self.goal_lists)

        # Whiten old pos + hightlight the chosen goal area
        if self.visualize:
        ...
        while (
            self._state in self.goal_lists
            or self._state in self.info_cells
            or not self._check_far_enough(self._state, self.goal_lists)
        ):
            self._state = random.randint(0, self.grid_len**2 - 1)
    ...
    def _color_goal_area(self, goal_loc, color="green"):
        ...
        if self.visualize:
            self.canvas.itemconfig(
                self._convert_coordinate(goal_loc[0], goal_loc[1]), fill=color
            )
    ...
    def _color(self, coordinates, color):
        ...
        for coord in coordinates:
            self.canvas.itemconfig(
                self._convert_coordinate(coord[0], coord[1]), fill=color
            )
