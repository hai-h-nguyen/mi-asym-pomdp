...
class SimpleReplayBuffer(ReplayBuffer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._next_obs = np.zeros((max_replay_buffer_size, observation_dim),
                                  np.float32)
        self._next_state = np.zeros((max_replay_buffer_size, state_dim),
                                    np.float32)
        ...
        self._expert = np.zeros((max_replay_buffer_size, 1), dtype="uint8")
    ...
    def sample_data(self, indices):
        return dict(
            ...
            state2=self._next_state[indices],
            expert=self._expert[indices],
        )
