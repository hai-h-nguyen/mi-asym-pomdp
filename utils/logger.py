...
def make_output_format(format, config, ev_dir, log_suffix=""):
    ...
    if format == "stdout":
        ...
        return WandBOutputFormat(config)
    else:
...
def save_to_cloud(*args):
    """
    save_to_cloud for wandb
    """
    Logger.CURRENT.save_to_cloud(*args)
...
class Logger(object):
    ...
    def __init__(self, dir, config, output_formats, precision=None):
    ...
    def set_tb_step(self, key, step):
        ...
        for fmt in self.output_formats:
            if isinstance(fmt, TensorBoardOutputFormat) \
               or isinstance(fmt, WandBOutputFormat):
                fmt.set_step(step)
    ...
    def save_to_cloud(self, path):
        for fmt in self.output_formats:
            if isinstance(fmt, WandBOutputFormat):
                fmt.save_to_cloud(path)
...
Logger.DEFAULT = Logger.CURRENT = Logger(
    dir=None, config=None, output_formats=[HumanOutputFormat(sys.stdout)]
)


def configure(dir, config, format_strs=LOG_OUTPUT_FORMATS, log_suffix="", precision=4):
    assert isinstance(dir, str)
    os.makedirs(dir, exist_ok=True)

    output_formats = [make_output_format(f, config, dir, log_suffix) for f in format_strs]

    Logger.CURRENT = Logger(dir=dir, config=config,
                            output_formats=output_formats, precision=precision)
    log("*" * 10, "\nLogging to %s" % dir, "\n" + "*" * 10)
